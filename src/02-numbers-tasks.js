...
function roundToPowerOfTen(a, b) {
  const check = `${10 ** b}`;
  const str = `${a}`;
  let round = str.substring(str.length - check.length);
  const reminder = str.substring(0, str.length - check.length);
  if (round.length === 1) {
    return reminder + round;
  }
  const zeros = '0'.repeat(round.length - 1);
  round = Math.round(`${round.slice(0, 1)}.${round.slice(1)}`);
  return reminder + round + zeros;
}
